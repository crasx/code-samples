	...
	if(is_numeric($c)){
		...
		if($cin=mysql_fetch_array(mysql_query("select name from ".T_COMPS." where id=$c and enabled=1 and day=curdate()"))){
			...
			if(isset($_GET['p'])){
				...
				$listc=mysql_query("select a.id, a.name, a.min, a.max, ifnull(s.score, a.min) score from ".T_CCLIST." l, ".T_CATS." a left join ".T_SCORES." s on( s.contestant=$p and s.judge=".$userInfo['id']. " and s.category=a.id) where l.competition=$c and l.category=a.id" );
				...
				if($que)if($r=mysql_fetch_array($que)){
					$body.="<a href='javascript:getHttp(\"ajax-php.php?page=judge&c=$c&p=".$r['id']."\", \"page\");waitLoadScroll();' >".$r['name']."</a> ";
				}
...
<table><tr><td valign=top >";
	if($save)$body.=" -- Last save: ".date("g:i:s");
				if($listc)while($tcat=mysql_fetch_array($listc)){
						$body.="<h3 style='float:left;'>".$tcat['name']."</h3>".getslider("cat".$tcat['id'], $tcat['min'], $tcat['max'], $tcat['score'],"cat[".$tcat['id']."]");
...
<br /><table border=1><tr><td>Description: </td><td>$contDes</td></tr>";
			...
			if($ppl)while($p=mysql_fetch_array($ppl)){
				$sco=false;
				$que=mysql_query(" SELECT count( s.category ) AS sco, (SELECT count( c.category ) AS cat FROM ".T_CCLIST." c WHERE c.competition =$c) AS cat FROM ".T_SCORES." s WHERE s.contestant =".$p['id']." AND s.judge =".$userInfo['id']." AND s.category IN (  SELECT category FROM ".T_CCLIST." WHERE competition =$c)");
				if($que)while($r=mysql_fetch_array($que)){
					$sco=$r['sco']==$r['cat'];
				}
				$body.= "<td>".$p['number']."</td><td><a href='javascript:getHttp(\"ajax-php.php?page=judge&c=$c&p=".$p['id']."\", \"page\");waitLoadScroll();' >".$p['name']."</a></td><td style='background-color:#".($sco?"00ff00":"ff0000")."'>&nbsp</td></tr>";
