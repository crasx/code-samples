...
$phpEx = substr(strrchr(__FILE__, './fop'), 1);
...
if (sizeof($topic_list))
{
	...
	foreach ($topic_list as $topic_id)
	{
		...
		$template->assign_block_vars('topicrow', array(
			...
			'POST_TEXT'			=>getPostText($forum_id, $topic_id),
...
function getPostText($forum, $topic){
	global $db;
	$sql="	SELECT po.post_text, po.bbcode_uid, po.bbcode_bitfield
FROM posts po
where po.forum_id=$forum
and po.topic_id=$topic
ORDER BY po.post_time ";
	$result = $db->sql_query($sql);
